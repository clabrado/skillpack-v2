...
description: >-
  ...
  autonomously through a fixed assess → triage → configure → analyze cycle
  until every technical deliverable the goal requires is genuinely complete,
  real artifacts checked, not claimed. Finishes with a full /readout printed
  to console and the same readout texted to Chris via iMessage. Triggers -
  "/standup", "stand up X", "standup <goal>".
argument-hint: "<goal description>"
---

# /standup — standing goal + assess/triage/configure/analyze loop + readout finish
...
Same shape as `/turbo`; the difference is the loop: standup runs a fixed
four-phase cycle (**assess → triage → configure → analyze**) each iteration,
rather than a free-form "do the next unit of work" step. Use standup when
the goal is standing something *up* — a service, a box, a pipeline, an
environment — where each pass is genuinely a survey/prioritize/change/verify
cycle. Use turbo when the work is open-ended build.

**Honest limitation, stated up front:** `/goal` is a built-in CLI primitive
(same family as `/clear`, `/compact`) — not a skill, not invocable
programmatically from inside another skill's instructions. Standup does not
...
Everything after `/standup` (or `standup:`) is the goal, verbatim. If it's
...
> Standup goal set: `<goal, verbatim>`. Working assess → triage → configure
> → analyze until every technical deliverable this requires is genuinely
> complete — will not stop, hand back, or ask "what next" before then.
...
- A tool-blocking or Stop-hook-style condition is not available here (see
  ...
  Chris "should I continue?" once he's already said `/standup`.
- If Chris sends something clearly unrelated mid-loop, handle it as an
  ...
  already built into this environment), but the standup goal remains active
  underneath unless he explicitly says to drop it or gives a new `/standup`.
- If real, external circumstances block progress (missing credential, a
  ...
  now — see the H15/GX10 precedent from 2026-08-21: he can pause a standup
...
## Step 3 — work the loop: assess → triage → configure → analyze

Each iteration runs all four phases in order. Never skip a phase silently:
if a phase is genuinely empty this pass, say so in one line and move on —
a measured empty, never an unmentioned skip.

**1. Assess.** Re-check actual state against the goal — `git log`, test
output, service health, ticket/board state, config on disk, whatever the
goal's domain is. Never trust your own memory of a prior iteration's
result; every claim is re-derived from an artifact this pass.

**2. Triage.** From what assess found, pick the *one* thing that most
blocks the goal, and say why it outranks the others in a sentence. Classify
it — defect, missing config, missing capability, external block. One root
class per pass (CLAUDE.md §3 / factory-ops triage discipline); if the last
two passes only fixed the previous pass's fix, that's a design problem —
escalate, don't patch-loop.

**3. Configure.** Do the real work on that one thing: write/fix code, change
the config, run the actual command. No stubs, no "should work". Respect the
safety rules — nothing deleted under the operator's home directory or app dirs, no
check-then-destroy in one chain, maintenance window announced to TRON before
any planned change that would trip it, and TRON's registry updated in the
same act that changes a registered component.

**4. Analyze.** Prove it, or don't claim it. Run the command that
demonstrates the change from the outside — as an operator would, not just a
green unit test — and read the actual output. State verified vs assumed.
An unmeasured field stays unmeasured; never invent a zero or a green.

Then decide: is every technical deliverable the goal requires now genuinely
done?
- **Not yet** → call `ScheduleWakeup` to continue: `prompt` is
  `/standup <goal, verbatim>` so a resumed session re-enters this same
...
- Doesn't replace `/turbo` for open-ended build work — standup's fixed
  four-phase cycle fits standing a system up, not free-form implementation.
...
- Doesn't replace `/loop <interval> <prompt>` for genuinely periodic,
  indefinite monitoring (e.g. "check the deploy every 20m forever") —
  standup is for a goal with a real finish line, not an unbounded watch.
...
- Doesn't skip the offer-cloud-first behavior `/loop` has for genuinely
  ...
  terminal closes.
- Is not a daily-standup / status-meeting generator, despite the name.
